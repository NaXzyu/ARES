...
from ares.utils.build.utils import (
    find_cython_binaries, find_sdl2_dlls
)
...
class ExeBuilder:
    ...
    def __init__(self, python_exe, output_dir, main_script, name=None, resources_dir=None, console_mode=True, onefile=True):
        ...
        self.executable_extension = '.exe' if is_windows() else ''
    ...
    def build(self):
        ...
        bins = find_sdl2_dlls(self.py_exe, log.info)
        cbins = find_cython_binaries(Paths.PROJECT_ROOT, log.info)
        bins.extend(cbins)

        # Create runtime hooks (using the HookManager class method)
        hooks = HookManager.create_runtime_hooks(self.output_path)

        # Use Paths.get_executable_output_dir to get the output directory
        ...
        spec_file = exe_spec.apply(bins=bins, hook_files=hooks)
        ...
        if target_exe.exists():
            ...
            raise RuntimeError(f"Error: Expected executable not found at {target_exe}")
