...
class ProjectConfig(BaseConfig):
    ...
    def get_company_name(self):
        ...
        # Fix parameter order: key, default, section
        return self.get("company_name", "Ares Engine", "project")

    def get_product_name(self):
        """Get the product name for the project."""
        # Fix parameter order: key, default, section
        return self.get("product_name", "Ares", "project")

    def get_file_description(self):
        """Get the file description for the project."""
        # Fix parameter order: key, default, section
        return self.get("file_description", "Game built with Ares Engine",
                        "project")

    def get_version_string(self):
        """Get full version string."""
        major = self.get_int("version", "major", 0)
        minor = self.get_int("version", "minor", 1)
        patch = self.get_int("version", "patch", 0)
